...
public class OnlineShoppingApp2 extends JFrame {
    ...
    void showPanel(String name) { cardLayout.show(mainPanel, name); }
    void setCustomerId(int id)  { this.customerId = id; }
...
class LoginForm extends JPanel {
    public LoginForm(OnlineShoppingApp2 app) {
        ...
        g.gridx=0; g.gridy=0; g.gridwidth=2; add(new JLabel("Login As:"), g);
        g.gridy=1; g.gridwidth=1;
        add(new JLabel("User Type:"), g); g.gridx=1; add(userTypeCombo, g);
        g.gridx=0; g.gridy=2; add(new JLabel("Username:"), g); g.gridx=1; add(usernameField, g);
        g.gridx=0; g.gridy=3; add(new JLabel("Password:"), g); g.gridx=1; add(passwordField, g);
        g.gridx=0; g.gridy=4; g.gridwidth=2; g.anchor=GridBagConstraints.CENTER;
        JPanel bp = new JPanel(); bp.add(loginBtn); bp.add(registerBtn); add(bp, g);
        ...
        loginBtn.addActionListener(e -> {
            ...
            if (app.con == null) { JOptionPane.showMessageDialog(this,"DB not connected"); return; }
            try {
                PreparedStatement st = app.con.prepareStatement(
                        "SELECT customer_id FROM Customerss WHERE name=? AND password=?");
                st.setString(1, user); st.setString(2, pass);
...
class RegistrationForm extends JPanel {
    public RegistrationForm(OnlineShoppingApp2 app) {
        ...
        g.gridx=0; g.gridy=0; add(new JLabel("Name:"),    g); g.gridx=1; add(nameField,  g);
        g.gridx=0; g.gridy=1; add(new JLabel("Email:"),   g); g.gridx=1; add(emailField, g);
        g.gridx=0; g.gridy=2; add(new JLabel("Phone:"),   g); g.gridx=1; add(phoneField, g);
        g.gridx=0; g.gridy=3; add(new JLabel("Password:"),g); g.gridx=1; add(passField,  g);
        g.gridx=0; g.gridy=4; add(new JLabel("Address:"), g); g.gridx=1; add(new JScrollPane(addressArea), g);
...
class CustomerDashboard extends JPanel {
    CustomerDashboard(OnlineShoppingApp2 app) {
        ...
        add(orderBtn); add(cartBtn); add(profileBtn); add(searchBtn); add(historyBtn);
...
class AdminDashboard extends JPanel {
    AdminDashboard(OnlineShoppingApp2 app) {
        ...
        add(addBtn); add(updateBtn); add(manageBtn);
...
class OrderPlacementForm extends JFrame {
    ...
    public OrderPlacementForm(OnlineShoppingApp2 app) {
        ...
        bottom.add(new JLabel("Quantity:")); bottom.add(quantityField);
        bottom.add(addToCartBtn); bottom.add(placeOrderBtn);
    ...
    private void addToCart(JTable table) {
        ...
        try {
            ...
            if (rs.next()) {
                ...
                create.setInt(1, app.getCustomerId()); create.executeUpdate();
                rs = getCart.executeQuery(); rs.next();
                cartId = rs.getInt("cart_id");
            }
            ...
            ins.setInt(1,cartId); ins.setInt(2,productId); ins.setInt(3,qty);
            ins.executeUpdate();
            JOptionPane.showMessageDialog(this,"Added to cart!");
            quantityField.setText("");
        } catch (NumberFormatException ex) { JOptionPane.showMessageDialog(this,"Enter a valid quantity"); }
        catch (SQLException ex)            { JOptionPane.showMessageDialog(this, ex.getMessage()); }
    }
    ...
    private void placeOrder() {
        try {
            ...
            getCart.setInt(1, app.getCustomerId());
            ResultSet rs = getCart.executeQuery();
            if (!rs.next()) { JOptionPane.showMessageDialog(this,"Cart is empty"); return; }
            ...
            ResultSet totalRs = totalSt.executeQuery(); totalRs.next();
            ...
            orderSt.setInt(1, app.getCustomerId()); orderSt.setDouble(2, total);
            ...
            getOrder.setInt(1,app.getCustomerId()); getOrder.setInt(2,app.getCustomerId());
            ResultSet ors = getOrder.executeQuery(); ors.next();
            ...
            while (ir.next()) {
                ...
                oiSt.setInt(1,orderId); oiSt.setInt(2,pid); oiSt.setInt(3,qty); oiSt.setDouble(4,price);
                oiSt.executeUpdate();
            }

            PreparedStatement clear = app.con.prepareStatement(
                    "DELETE FROM CartItemss WHERE cart_id=?");
            clear.setInt(1,cartId); clear.executeUpdate();
...
class ShoppingCartForm extends JFrame {
    ...
    public ShoppingCartForm(OnlineShoppingApp2 app) {
        ...
        btnPanel.add(updateBtn); btnPanel.add(removeBtn); btnPanel.add(placeOrderBtn);
    ...
    private void loadCartItems() {
        try {
            ...
            getCart.setInt(1,app.getCustomerId());
            ResultSet rs = getCart.executeQuery();
            if (!rs.next()) { cartModel.setRowCount(0); totalLabel.setText("Total: 0.00"); return; }
            int cartId = rs.getInt("cart_id");
            ...
            double total = 0;
            while (r.next()) {
                double sub = r.getDouble("subtotal"); total += sub;
    ...
    private void updateQuantity(JTable cartTable) {
        ...
        try {
            ...
            st.setInt(1,qty); st.setInt(2,cartItemId); st.executeUpdate();
            loadCartItems();
            JOptionPane.showMessageDialog(this,"Quantity updated");
        } catch (NumberFormatException ex) { JOptionPane.showMessageDialog(this,"Enter a valid number"); }
        catch (SQLException ex)            { JOptionPane.showMessageDialog(this, ex.getMessage()); }
    }
    ...
    private void removeItem(JTable cartTable) {
        ...
        try {
            ...
            st.setInt(1,cartItemId); st.executeUpdate();
            loadCartItems();
    ...
    private void placeOrderFromCart() {
        try {
            ...
            PreparedStatement totalSt = app.con.prepareStatement(
                    ...
                    "JOIN Productss p ON ci.product_id=p.product_id WHERE ci.cart_id=?");
            totalSt.setInt(1,cartId);
            ResultSet tr = totalSt.executeQuery(); tr.next();
            ...
            PreparedStatement getOrder = app.con.prepareStatement(
                    "SELECT order_id FROM Orderss WHERE customer_id=? AND order_date=(SELECT MAX(order_date) FROM Orderss WHERE customer_id=?)");
            getOrder.setInt(1,app.getCustomerId()); getOrder.setInt(2,app.getCustomerId());
            ResultSet or = getOrder.executeQuery(); or.next();
            ...
            while (ir.next()) {
                ...
                PreparedStatement pr = app.con.prepareStatement(
                        "SELECT price FROM Productss WHERE product_id=?");
                pr.setInt(1,pid);
                ResultSet prRs = pr.executeQuery(); prRs.next();
                double price = prRs.getDouble("price");

                PreparedStatement oi = app.con.prepareStatement(
                        "INSERT INTO OrderItemss (order_item_id,order_id,product_id,quantity,price) " +
                        "VALUES (orderitemss_seq.NEXTVAL,?,?,?,?)");
                oi.setInt(1,orderId); oi.setInt(2,pid); oi.setInt(3,qty); oi.setDouble(4,price);
                oi.executeUpdate();
            }
...
class CustomerProfileUpdateForm extends JFrame {
    ...
    public CustomerProfileUpdateForm(OnlineShoppingApp2 app) {
        ...
        g.gridx=0; g.gridy=1; add(new JLabel("Email:"),   g); g.gridx=1; add(emailField, g);
        g.gridx=0; g.gridy=2; add(new JLabel("Phone:"),   g); g.gridx=1; add(phoneField, g);
        g.gridx=0; g.gridy=3; add(new JLabel("Address:"), g); g.gridx=1; add(new JScrollPane(addressArea), g);
    ...
    private void updateProfile() {
        ...
        try {
            ...
            st.setString(1,nameField.getText()); st.setString(2,emailField.getText());
            st.setString(3,phoneField.getText()); st.setString(4,addressArea.getText());
...
class ProductSearchForm extends JFrame {
    ...
    public ProductSearchForm(OnlineShoppingApp2 app) {
        ...
        top.add(new JLabel("Search:")); top.add(searchTypeCombo);
...
class OrderHistoryPage extends JFrame {
    private OnlineShoppingApp2 app;
    private DefaultTableModel orderModel, itemModel;
    ...
    public OrderHistoryPage(OnlineShoppingApp2 app) {
        ...
        ordersPanel.add(new JLabel("Your Orders:"),    BorderLayout.NORTH);
        ...
        JSplitPane split = new JSplitPane(JSplitPane.HORIZONTAL_SPLIT, ordersPanel, itemsPanel);
        split.setDividerLocation(400);
        add(split, BorderLayout.CENTER);

        orderTable.getSelectionModel().addListSelectionListener(e -> {
            int row = orderTable.getSelectedRow();
            if (row == -1) { itemModel.setRowCount(0); return; }
            try {
                ...
                while (rs.next())
                    itemModel.addRow(new Object[]{rs.getString("product_name"),
    ...
    private void loadOrderHistory() {
        try {
            PreparedStatement st = app.con.prepareStatement(
                    "SELECT order_id,order_date,total_amount,order_status FROM Orderss " +
                    "WHERE customer_id=? ORDER BY order_date DESC");
            st.setInt(1,app.getCustomerId());
            ResultSet rs = st.executeQuery();
            orderModel.setRowCount(0);
            while (rs.next())
                orderModel.addRow(new Object[]{rs.getInt("order_id"), rs.getDate("order_date"),
...
class AddProductForm extends JFrame {
    public AddProductForm(OnlineShoppingApp2 app) {
        ...
        g.gridx=0; g.gridy=0; add(new JLabel("Product Name:"), g); g.gridx=1; add(nameField,  g);
        g.gridx=0; g.gridy=1; add(new JLabel("Category:"),     g); g.gridx=1; add(catField,   g);
        g.gridx=0; g.gridy=2; add(new JLabel("Price:"),        g); g.gridx=1; add(priceField, g);
        g.gridx=0; g.gridy=3; add(new JLabel("Stock:"),        g); g.gridx=1; add(stockField, g);

        JButton addBtn = new JButton("Add Product");
        g.gridx=0; g.gridy=4; g.gridwidth=2; add(addBtn, g);
        ...
        addBtn.addActionListener(e -> {
            try {
                ...
                st.setString(1,nameField.getText()); st.setString(2,catField.getText());
                st.setDouble(3,Double.parseDouble(priceField.getText()));
                st.setInt(4,Integer.parseInt(stockField.getText()));
                st.executeUpdate();
                JOptionPane.showMessageDialog(this,"Product added successfully!");
                dispose();
            } catch (NumberFormatException ex) { JOptionPane.showMessageDialog(this,"Enter valid price and stock"); }
            catch (SQLException ex)            { JOptionPane.showMessageDialog(this, ex.getMessage()); }
        });
    }
}
...
class UpdateProductForm extends JFrame {
    ...
    public UpdateProductForm(OnlineShoppingApp2 app) {
        ...
        nameField  = new JTextField(15); catField   = new JTextField(15);
        priceField = new JTextField(15); stockField = new JTextField(15);

        g.gridx=0; g.gridy=0; add(new JLabel("Select Product:"), g); g.gridx=1; add(productCombo, g);
        g.gridx=0; g.gridy=1; add(new JLabel("Name:"),     g); g.gridx=1; add(nameField,  g);
        g.gridx=0; g.gridy=2; add(new JLabel("Category:"), g); g.gridx=1; add(catField,   g);
        g.gridx=0; g.gridy=3; add(new JLabel("Price:"),    g); g.gridx=1; add(priceField, g);
        g.gridx=0; g.gridy=4; add(new JLabel("Stock:"),    g); g.gridx=1; add(stockField, g);

        JButton updateBtn = new JButton("Update Product");
        g.gridx=0; g.gridy=5; g.gridwidth=2; add(updateBtn, g);

        loading = true; loadProductsIntoCombo(); loading = false;
        if (productCombo.getItemCount() > 0) loadProductDetails();

        productCombo.addActionListener(e -> { if (!loading) loadProductDetails(); });
        ...
        updateBtn.addActionListener(e -> {
            try {
                ...
                st.setString(1,nameField.getText()); st.setString(2,catField.getText());
                st.setDouble(3,Double.parseDouble(priceField.getText()));
                st.setInt(4,Integer.parseInt(stockField.getText())); st.setInt(5,pid);
                st.executeUpdate();
...
class ManageProductsForm extends JFrame {
    ...
    public ManageProductsForm(OnlineShoppingApp2 app) {
        ...
        bottom.add(deleteBtn); bottom.add(refreshBtn);
        add(bottom, BorderLayout.SOUTH);
